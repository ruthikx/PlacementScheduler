...
import { formatSlotTime } from './replanner';

export function validateSchedule(state: ScheduleState): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];

  const studentBusy: { [studentId: string]: { [day: number]: { [slot: number]: string } } } = {};
  const roomBusy: { [roomName: string]: { [day: number]: { [slot: number]: string } } } = {};
  const panelBusy: { [companyPanel: string]: { [day: number]: { [slot: number]: string } } } = {};
  ...
  state.interviews.forEach(interview => {
    ...
    for (let s = start; s < start + duration; s++) {
      // Student check
      if (!studentBusy[student.id]) studentBusy[student.id] = {};
      if (!studentBusy[student.id][day]) studentBusy[student.id][day] = {};
      if (studentBusy[student.id][day][s]) {
        errors.push(`Student Double Booking: Student ${student.name} is booked for both "${studentBusy[student.id][day][s]}" and "${company.name}" on ${formatSlotTime(day, s)}.`);
      } else {
        studentBusy[student.id][day][s] = company.name;
      }

      // Room check
      if (!roomBusy[roomName]) roomBusy[roomName] = {};
      if (!roomBusy[roomName][day]) roomBusy[roomName][day] = {};
      if (roomBusy[roomName][day][s]) {
        errors.push(`Room Double Booking: Room ${roomName} is occupied by both "${roomBusy[roomName][day][s]}" and "${company.name}" on ${formatSlotTime(day, s)}.`);
      } else {
        roomBusy[roomName][day][s] = company.name;
      }

      // Panel check
      if (!panelBusy[panelKey]) panelBusy[panelKey] = {};
      if (!panelBusy[panelKey][day]) panelBusy[panelKey][day] = {};
      if (panelBusy[panelKey][day][s]) {
        errors.push(`Panel Double Booking: Panel ${interview.panelIndex + 1} of ${company.name} is booked for both student ID "${panelBusy[panelKey][day][s]}" and student ID "${student.name}" on ${formatSlotTime(day, s)}.`);
      } else {
