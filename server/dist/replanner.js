...
function formatSlotTime(day, slotIndex) {
...
class Replanner {
    ...
    initBusyGrids(activeInterviews) {
        this.studentBusy = {};
        this.roomBusy = {};
        this.panelBusy = {};
    ...
    getAffectedInterviews(disruption) {
        switch (disruption.type) {
            case 'COMPANY_LATE': {
                const { companyId, day, hoursLate } = disruption;
                const slotsLate = (hoursLate || 0) * 2;
                return this.interviews.filter(i => i.companyId === companyId &&
                    i.day === day &&
            ...
            case 'ROOM_UNAVAILABLE': {
                ...
                return this.interviews.filter(i => i.roomName === roomName &&
                    i.day === day &&
    ...
    replan(disruption, policy, maxChurn) {
                        ...
                        for (let startSlot = 0; startSlot <= 18 - c.durationSlots; startSlot++) {
                            ...
                            for (let pIdx = 0; pIdx < c.panelsCount; pIdx++) {
                                ...
                                for (const room of this.rooms) {
                                    if (this.isStudentFree(s.id, day, startSlot, c.durationSlots) &&
        ...
        const failedToReschedule = [];
        const relocatedUnaffected = [];
        ...
        for (const interview of sortedAffected) {
                ...
                for (let startSlot = 0; startSlot <= 18 - comp.durationSlots; startSlot++) {
                    ...
                    for (let pIdx = 0; pIdx < comp.panelsCount; pIdx++) {
                        ...
                        for (const room of this.rooms) {
                            ...
                            if (this.isStudentFree(stud.id, day, startSlot, comp.durationSlots) &&
            ...
            if (!scheduled && relocatedUnaffected.length < maxChurn) {
                ...
                for (const otherInt of unaffectedInterviews) {
                    ...
                    if (comp.tier === 'mass' && otherComp.tier === 'niche')
                        continue;
                    ...
                    if (this.isStudentFree(stud.id, otherDay, otherSlot, comp.durationSlots) &&
                            ...
                            for (let s = 0; s <= 18 - otherInt.durationSlots; s++) {
                                ...
                                for (let p = 0; p < otherComp.panelsCount; p++) {
                                    ...
                                    for (const r of this.rooms) {
                                        const overlapsOriginalTime = d === otherDay &&
                                        ...
                                        if (this.isStudentFree(otherInt.studentId, d, s, otherInt.durationSlots) &&
                ...
                if (policy === 'EXTEND_DAY') {
                    ...
                    for (let i = 0; i < finalInterviews.length; i++) {
                        ...
                        if (fiComp.tier === 'mass') {
                            ...
                            // Check if now free
                            if (this.isStudentFree(stud.id, fi.day, fi.startSlot, comp.durationSlots) &&
    ...
    isSlotDisrupted(disruption, companyId, panelIndex, roomName, day, startSlot, duration) {
        const s = startSlot || 0;
        const d = duration || 1;
        switch (disruption.type) {
            case 'COMPANY_LATE': {
                ...
                if (companyId === discCompId && day === discDay) {
                    if (startSlot === undefined)
                        return true; // block entire day check
                    return s < slotsLate;
                }
                return false;
            }
    ...
    getDisruptionReason(disruption) {
        switch (disruption.type) {
            case 'COMPANY_LATE':
